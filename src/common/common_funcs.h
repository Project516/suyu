...
#define DECLARE_ENUM_FLAG_OPERATORS(type)                                                          \
    ...
    constexpr type operator|=(type& a, type b) noexcept { return a = a | b; }                      \
    constexpr type operator&=(type& a, type b) noexcept { return a = a & b; }                      \
    constexpr type operator^=(type& a, type b) noexcept { return a = a ^ b; }                      \
    constexpr type operator<<=(type& a, type b) noexcept { return a = a << b; }                    \
    constexpr type operator>>=(type& a, type b) noexcept { return a = a >> b; }                    \
    ...
    [[nodiscard]] constexpr bool True(type key) noexcept { return std::to_underlying(key) != 0; }  \
    [[nodiscard]] constexpr bool False(type key) noexcept { return std::to_underlying(key) == 0; }
