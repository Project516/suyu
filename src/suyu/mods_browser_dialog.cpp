...
ModBrowserDialog::ModBrowserDialog(QWidget* parent) : QDialog(parent) {
    ...
    QAction* reload_action =
        toolbar->addAction(style()->standardIcon(QStyle::SP_BrowserReload), tr("Reload"));
    ...
    connect(open_external_action, &QAction::triggered, this,
            [web_view] { QDesktopServices::openUrl(web_view->url()); });
    connect(web_view, &QWebEngineView::urlChanged, this,
            [url_label](const QUrl& url) { url_label->setText(url.toString()); });
#else
    auto* layout = new QVBoxLayout(this);
    auto* message = new QLabel(tr("Qt WebEngine is not available in this build. Install Qt "
                                  "WebEngine to browse mods inside the app."),
                               this);
