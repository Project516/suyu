...
void DecryptionFlowWidget::DetectKeys() {
    ...
    const bool has_prod_keys =
        (prod.exists() && prod.size() > 0) || (prod_auto.exists() && prod_auto.size() > 0);
    const bool has_title_keys = title.exists() || title_auto.exists();

    keys_valid_ = has_prod_keys;
    if (keys_valid_) {
        lbl_status_->setText(QStringLiteral(
            "<span style='color:green;font-size:14pt;'>Decryption keys detected</span>"));
        lbl_instructions_->setText(
            QStringLiteral(
                "<p><b>prod.keys</b> is installed in <b>%1</b>.</p>"
                "<p>%2</p>"
        ...
        lbl_status_->setText(QStringLiteral(
            "<span style='color:#c0392b;font-size:14pt;'>Decryption keys not found</span>"));
        lbl_instructions_->setText(
            QStringLiteral(
                "<p>Install <b>prod.keys</b> into <b>%1</b> using <b>Tools -> Install Decryption "
                "Keys</b>.</p>"
                "<p>You can still configure an external decryption tool if that workflow suits you "
                "better.</p>"
                "<p><i>suyu does not provide keys. Dump keys from hardware you own.</i></p>")
                .arg(QDir::toNativeSeparators(keys_dir.absolutePath())));
    }
