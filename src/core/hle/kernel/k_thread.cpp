...
Result KThread::Initialize(KernelCore& kernel, KThreadFunction func, uintptr_t arg,
...
Result KThread::InitializeThread(KernelCore& kernel, KThread* thread, KThreadFunction func,
                                 uintptr_t arg, KProcessAddress user_stack_top, s32 prio, s32 core,
...
Result KThread::InitializeDummyThread(Core::System& system, KThread* thread, KProcess* owner) {
    // Initialize the thread.
    R_TRY(thread->Initialize(system.Kernel(), {}, {}, {}, DummyThreadPriority, 3, owner,
                             ThreadType::Dummy));
...
Result KThread::InitializeMainThread(Core::System& system, KThread* thread, s32 virt_core) {
    R_RETURN(InitializeThread(system.Kernel(), thread, {}, {}, {}, IdleThreadPriority, virt_core,
                              {}, ThreadType::Main,
                              system.GetCpuManager().GetGuestActivateFunc(system.Kernel())));
}

Result KThread::InitializeIdleThread(Core::System& system, KThread* thread, s32 virt_core) {
    R_RETURN(InitializeThread(system.Kernel(), thread, {}, {}, {}, IdleThreadPriority, virt_core,
                              {}, ThreadType::Main,
                              system.GetCpuManager().GetIdleThreadStartFunc(system.Kernel())));
}
...
Result KThread::InitializeHighPriorityThread(Core::System& system, KThread* thread,
    ...
    R_RETURN(InitializeThread(system.Kernel(), thread, func, arg, {}, {}, virt_core, nullptr,
                              ...
                              system.GetCpuManager().GetShutdownThreadStartFunc(system.Kernel())));
}
...
Result KThread::InitializeUserThread(Core::System& system, KThread* thread, KThreadFunction func,
    ...
    R_RETURN(InitializeThread(system.Kernel(), thread, func, arg, user_stack_top, prio, virt_core,
                              owner, ThreadType::User,
                              system.GetCpuManager().GetGuestThreadFunc(system.Kernel())));
}
...
Result KThread::InitializeServiceThread(Core::System& system, KThread* thread,
    ...
    R_RETURN(InitializeThread(system.Kernel(), thread, {}, {}, {}, prio, virt_core, owner,
                              ThreadType::HighPriority, std::move(func2)));
}
...
void KThread::PostDestroy(KernelCore& kernel, uintptr_t arg) {
    ...
    if (owner != nullptr) {
        owner->GetResourceLimit()->Release(kernel, LimitableResource::ThreadCountMax, 1,
                                           hint_value);
        owner->Close(kernel);
    }
}
...
void KThread::Pin(KernelCore& kernel, s32 current_core) {
    ...
    ASSERT(m_num_core_migration_disables == 0);
    {
        ...
        if (active_core != current_core ||
            ...
            KScheduler::OnThreadAffinityMaskChanged(kernel, this, m_original_physical_affinity_mask,
                                                    active_core);
        }
    }
...
Result KThread::GetPhysicalCoreMask(KernelCore& kernel, s32* out_ideal_core,
...
KThread::LockWithPriorityInheritanceInfo* KThread::FindHeldLock(KernelCore& kernel,
...
KThread* KThread::RemoveWaiterByKey(KernelCore& kernel, bool* out_has_waiters, KProcessAddress key,
...
void KThread::Exit(KernelCore& kernel) {
    ...
    if (m_parent != nullptr) {
        m_parent->GetResourceLimit()->Release(kernel, Kernel::LimitableResource::ThreadCountMax, 0,
                                              1);
...
Result KThread::Terminate(KernelCore& kernel) {
    ...
    if (const auto new_state = this->RequestTerminate(kernel);
...
void KThread::NotifyAvailable(KernelCore& kernel, KSynchronizationObject* signaled_object,
                              Result wait_result) {
    // Lock the scheduler.
    KScopedSchedulerLock sl(kernel);
