...
Result IHidServer::SetSupportedNpadStyleSet(Core::HID::NpadStyleSet supported_style_set,
    ...
    R_TRY(GetResourceManager()->GetNpad()->SetSupportedNpadStyleSet(system.Kernel(), aruid.pid,
                                                                    supported_style_set));
...
Result IHidServer::SetSupportedNpadIdType(
    ...
    R_RETURN(GetResourceManager()->GetNpad()->SetSupportedNpadIdType(system.Kernel(), aruid.pid,
                                                                     supported_npad_list));
}
...
Result IHidServer::AcquireNpadStyleSetUpdateEventHandle(
    ...
    R_RETURN(GetResourceManager()->GetNpad()->AcquireNpadStyleSetUpdateEventHandle(
        system.Kernel(), aruid.pid, out_event, npad_id));
}
...
Result IHidServer::MergeSingleJoyAsDualJoy(Core::HID::NpadIdType npad_id_1,
    ...
    R_RETURN(GetResourceManager()->GetNpad()->MergeSingleJoyAsDualJoy(system.Kernel(), aruid.pid,
                                                                      npad_id_1, npad_id_2));
}
...
Result IHidServer::SetNpadHandheldActivationMode(ClientAppletResourceUserId aruid,
    ...
    R_RETURN(GetResourceManager()->GetNpad()->SetNpadHandheldActivationMode(
        system.Kernel(), aruid.pid, activation_mode));
}
...
Result IHidServer::GetNpadHandheldActivationMode(
    ...
    LOG_DEBUG(Service_HID, "called, applet_resource_user_id={}", aruid.pid);
    R_RETURN(GetResourceManager()->GetNpad()->GetNpadHandheldActivationMode(
        system.Kernel(), aruid.pid, *out_activation_mode));
}
...
Result IHidServer::SwapNpadAssignment(Core::HID::NpadIdType npad_id_1,
    ...
    LOG_DEBUG(Service_HID, "called, npad_id_1={}, npad_id_2={}, applet_resource_user_id={}",
              npad_id_1, npad_id_2, aruid.pid);
    R_RETURN(GetResourceManager()->GetNpad()->SwapNpadAssignment(system.Kernel(), aruid.pid,
                                                                 npad_id_1, npad_id_2))
}
...
Result IHidServer::SetNpadJoyAssignmentModeSingleWithDestination(
    ...
    *out_is_reassigned = GetResourceManager()->GetNpad()->SetNpadMode(
        system.Kernel(), aruid.pid, *out_new_npad_id, npad_id, npad_joy_device_type,
        NpadJoyAssignmentMode::Single);
    R_SUCCEED();
}
